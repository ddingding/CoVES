...
def make_mut_seq_from_mut_key_and_wt_seq(mut_key, wt_seq):
    ...
    assert hamming(final_mut_seq, wt_seq) == len(muts) # check only desired mutations introduced
    assert len(final_mut_seq) == len(wt_seq) # check no error in making the full mutated sequence
...
def fit_plot_train_test(df_train, df_test, 
                        fit_val = 'fitness', 
                        xlim= None,
                        ylim= None, 
                        lr = 0.005, 
...
def run_subsampling(prot_n, 
    ...
    for fold in fold_obs_test:
        ...
        for i in range(n_reps):
            df_train, df_test = get_df_train_test(df_dms, train_frac = fraction_test, rand_seed = i)
            df_train, df_test, model_subset= fit_plot_train_test(
                                                    ...
                                                    batch_size=batch_size, 
                                                    layer = layer,
                                                    chpt_id = str(prot_n+'_'+str(i)+'_'+str(int(time.time()))) # to make unique checkpoint
                                                    )
...
def plot_corr_marginal(x, y, 
    ...
    if plot_diag:
        ax2.plot(
            ...
            [min(xticks)-plot_margin, max(xticks)+plot_margin], 
            color=diag_col, alpha=diag_alpha,
            )
...
def plot_roc(df_yhats_y, 
    ...
    for t_norm in np.array(list(range(n_thresh))+[n_thresh])/n_thresh:
        ...
        t = (t_norm*(max(df_yhats_y.yhat_non) - min(df_yhats_y.yhat_non))) + min(df_yhats_y.yhat_non)
...
def plot_roc(df_yhats_y, 
    ...
    for t in np.array(list(range(n_thresh))+[n_thresh])/n_thresh:
        #TPR: P(true positive| true positive + FN), or sensitivity
        df_ground_truth_positive = df_yhats_y.loc[df_yhats_y.yobs>true_thresh]
        df_TP = df_ground_truth_positive.loc[df_ground_truth_positive.yhat_non>t]
        tpr = len(df_TP)/len(df_ground_truth_positive)
    ...
    plt.xlabel('FPR (1-Specificity)', fontsize=8)
    plt.tight_layout()
    plt.savefig(dout + fname +'.svg', format='svg')
...
'''
alphabet= list("ACDEFGHIKLMNPQRSTVWY_")
d_aa_to_oh_pos  = dict(zip(alphabet, list(range(len(alphabet)))))
d_oh_pos_to_aa  = dict(zip( list(range(len(alphabet))), alphabet))
'''

'''
if use_stop == False:
    return idx_to_aa(pos)
else:
    return idx_to_aa_stop(pos)
'''
'''
# convert an amino acid to one-hot encoding
if use_stop == False:
    oh = np.zeros(len(AA_ALPHABET))
    oh[aa_to_idx[aa]] = 1
else:
    oh = np.zeros(len(AA_ALPHABET_STOP))
    oh[aa_to_idx_stop[aa]] = 1 
return oh
'''
'''
def get_oh_for_pos(aa, dic_mut_to_oh_pos):
    oh = np.zeros(len(dic_mut_to_oh_pos))
    oh[dic_mut_to_oh_pos[aa]] = 1
    return oh


def aa_to_oh(aa, use_stop = False):
    # convert an amino acid to one-hot encoding
    if use_stop == False:
        oh = np.zeros(len(AA_ALPHABET))
        oh[aa_to_idx[aa]] = 1
...
def mut_to_oh_all(aa, d_aa_to_oh_pos):
    oh = np.zeros(len(d_aa_to_oh_pos))
    oh[d_aa_to_oh_pos[aa]] = 1
    return oh

'''

'''
# these were used for the antitoxin 
def get_subset_preds(x,y, i = 3, subset=0.9, lr = 0.1):
    # splitting data randomly into test and train sets
    # the training on train set, and predicting on the full dataset, or just the test set
    
    i_to_pred = {}

    for i in range(i):
        # split up into test and train
        x_train, y_train, x_test, y_test = get_subset_data(x,y, subset=subset)
        ...
        if len(x_test) != 0:
            ...
            var_exp_test = np.nan
            pearsonr_test = np.nan
            spearmanr_test = np.nan
            
        # for the full dataset X
        yhat_all= model.predict(x).flatten()
        var_exp_all, pearsonr_all, spearmanr_all = get_corr_metrics(yhat_all, y)

        # store
        print(var_exp_train, var_exp_test, var_exp_all, pearsonr, spearmanr)
        corr_metrics = (var_exp_train, pearsonr_train, spearmanr_train,
                        var_exp_test, pearsonr_test, spearmanr_test,
                        var_exp_all, pearsonr_all, spearmanr_all)
        print(corr_metrics)
        i_to_pred[i] = corr_metrics

    return i_to_pred

#
def get_all_n_to_var(x,y, 
                     subsets=[0.9], 
                     n_iter=1, 
                     n_pos_rand=0, 
                     dout = './'):
    # for a particular dataset x,y
    # subsample and then predict on remaining, all or train set, for multiple iterations
    # then write this out
    
    cols = ['pos_rand', 'i', 'subset_train', 
           'var_exp_train', 'pears_train', 'spear_train', 
           'var_exp_test', 'pears_test', 'spear_test',
           'var_exp_train', 'pears_train', 'spear_train']
    df_met= pd.DataFrame(columns = cols)
    
    for sub in subsets:
        i_to_pred = get_subset_preds(x,y, i = n_iter, subset=sub)
        for i, metrics in i_to_pred.items():
            row_to_add = (n_pos_rand, i, sub) + metrics
            df_met.loc[len(df_met)] = row_to_add
            df_met.to_csv(dout + 'subsample_metrics_pos{}_iters{}.csv'.format(n_pos_rand, n_iter))
'''            
'''
# use seq_to_oh instead and concatenate the bias term instead np.concatenate([full_oh, np.array([1])])
def create_oh_mut(mut_str, use_stop = False):
    # mut_str (like 'L47L:D51D:'), and creates a one-hot vector 
    # with a bias term that matches the number of positions int

    # adding all oh terms for each position
    full_oh = []
    for mut in mut_str.split(':'):
        mut_aa = mut[-1]
        full_oh = np.concatenate([full_oh,aa_to_oh(mut_aa, use_stop=use_stop)])
    # adding a bias term at end
    full_oh = np.concatenate([full_oh, np.array([1])])
    return full_oh
# use seq_to_oh_instead
